...
def test_board_to_tensor_pieces() -> None:
    ...
    assert aux[1].item() == 1.0          # white kingside
    assert aux[2].item() == 1.0          # white queenside
    assert aux[3].item() == 1.0          # black kingside
    assert aux[4].item() == 1.0          # black queenside
...
def test_promotion_encoding() -> None:
    # only push promotions are legal; the black king keeps the e-file clear.
    ...
    # capture promotions need diagonal targets.
    ...
    # black underpromotion recovers rank delta from board.turn.
